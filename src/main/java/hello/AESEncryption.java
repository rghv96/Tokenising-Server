...
public class AESEncryption {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String k = bytesToHex(secKey.getEncoded());
        String enc_card = bytesToHex(cipherText);
        System.out.println("Original Text:" + plainText);
        System.out.println("AES Key (Hex Form):"+k);
        System.out.println("Encrypted Text (Hex Form):"+enc_card);

        byte[] cipher = enc_card.getBytes();
        // decode the base64 encoded string
        byte[] decodedKey = k.getBytes();
        //System.out.println("Byte array: " + Arrays.toString(decodedKey));
        // rebuild key using SecretKeySpec
        SecretKey sexcKey = new SecretKeySpec(decodedKey, 0, decodedKey.length, "AES");
        String decryptedText = decryptText(cipher, sexcKey);
