...
public class MainController {
    	...
    	if(card.length()>=11 && card.length()<=16)
		{
			while(true)
			{
				for(int i=0;i<card.length();i++)
				{
					if(ch[i]<'0' || ch[i]>'9')
					{
						Token t = new Token("Error, Card should contain only numbers", card);
						return t;
					}
			...
			//System.out.println("AES Key (Hex Form):" + obj.bytesToHex(secKey.getEncoded()));
			passkey = obj.bytesToHex(secKey.getEncoded());
			String enc_card = obj.bytesToHex(cipherText);

			enc_card += lastfour;
			Token t = new Token(token, enc_card);
			tokenRepository.save(t);

			d.setToken(token);
			d.setPasskey(passkey);
			//System.out.println("Key: " + passkey);
			dsmRepository.save(d);
			return t;
		}
		
		Token t = new Token("Error, Card length should be 11 to 16", card);
		return t;
	}
	...
	public @ResponseBody Token getToken (@RequestParam String token) {
		...
		Token ans = new Token(token, decryptedText);
		return ans;
	}
